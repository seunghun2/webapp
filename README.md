...
- ✅ **정렬 옵션**: 마감임박일(가까운순) / 마감일(먼순) 2가지
- ✅ **스마트 정렬**: steps 기반 프론트엔드 재정렬로 정확한 마감일 순서
- ✅ **스크롤 투 탑**: 흰색 배경에 회색 테두리의 맨 위로 버튼
...
- ✅ **PDF 자동 파싱**: AI 기반 분양 공고 PDF 자동 추출 및 양식 자동 입력
...
- **2025-01-28**: 햄버거 바 및 로그인 모달 공통 컴포넌트화
  - getLoginModal(), getHamburgerMenu(), getCommonScripts() 함수 생성
  - Calculator, Savings, Rates 페이지에 공통 컴포넌트 적용
  - 코드 중복 제거로 번들 크기 4KB 감소 (810KB → 806KB)
  - 전체 페이지 햄버거 바 통일 및 알림설정 로그인 팝업 연동
- **2025-01-28**: 로컬/배포 서버 복구 완료
  - Git 롤백 (커밋 80ec691)으로 안정 버전 복원
  - 로컬 DB 마이그레이션 및 테이블 구조 복구
  - 이미지 저장 기능 수정 (image_url 필드 추가)
- **2025-01-28**: PDF 자동 파싱 기능 작동 확인 ✅
  - AI 기반 분양 공고 PDF 자동 추출
  - 프로덕션 환경에서 정상 작동 확인
...
- **2025-11-20**: 정렬 옵션 명칭 변경 (마감임박일/마감일)
- **2025-11-20**: 스크롤 투 탑 버튼 추가 (300px 이상 스크롤 시 표시)
- **2025-11-20**: 모바일 필터링 수정 (지역/세대수/평형 필터 적용)
- **2025-11-19**: 이미지 표시 수정 (어드민 저장 이미지 UI 반영)
- **2025-11-19**: 카드 썸네일 위치 조정 (제목/위치 아래로 이동)
- **2025-11-19**: 네이버 지도 연동 (카카오 지도 → 네이버 지도)
